...
const Chatbot = () => {
  ...
  const isValidEmail = (email) =>
    /^[a-zA-Z0-9._%+-]+@(gmail\.com|test\.com)$/.test(email);
  ...
  // Memoize the toggleChatbot function
  ...
  useEffect(() => {
    // Automatically open the chatbot when the component is mounted
    if (!isOpen) {
      toggleChatbot();
    }
  ...
  const handleClose = async () => {
    try {
      ...
      setConversation((prev) => [
        ...prev,
        { text: response.data.message, isBot: true, options: ["Y", "N"] },
      ]);
    } catch (error) {
      ...
      setConversation((prev) => [
        ...prev,
        {
          text: "Error terminating the conversation. Please try again later.",
          isBot: true,
          options: [],
        },
      ]);
      ...
      if (["Y", "N"].includes(option)) {
        ...
        if (response?.data?.error) {
          ...
          setConversation((prev) => [
            ...prev,
            {
              text: "Invalid choice. Please select a valid option.",
              isBot: true,
              options: [],
            },
          ]);
        ...
        if (option === "Our Services") {
          ...
          setConversation((prev) => [
            ...prev,
            {
              text: "When do you wish to start?",
              isBot: true,
          ...
          setConversation((prev) => [
            ...prev,
            {
              text: "Kindly provide your details to help us provide you the best service:",
              isBot: true,
            },
            { text: "Please provide your name.", isBot: true },
          ]);
          ...
          option === "Book A Demo" ||
          response.data.message?.includes("Kindly provide your details")
        ) {
          setConversation((prev) => [
            ...prev,
            {
    ...
    if (currentStep === 1) {
      ...
      if (!isValid) {
        setConversation((prev) => [
          ...prev,
          {
            text: "Invalid name. Please enter alphabetic characters only.",
            isBot: true,
          },
        ]);
        return;
      ...
      if (!isValid) {
        setConversation((prev) => [
          ...prev,
          {
            text: "Invalid email. Please enter a valid email with @gmail.com or @test.com.",
            isBot: true,
          },
        ]);
        return;
      ...
      setConversation((prev) => [
        ...
        { text: detail, isBot: false, isUser: true },
        {
          text: "Please provide your phone number (without country code).",
          isBot: true,
        },
      ]);
      setCurrentStep(3);
      ...
      if (!isValid) {
        setConversation((prev) => [
          ...prev,
          {
            text: "Invalid phone number. Please enter a 10-digit number starting with 6-9.",
            isBot: true,
          },
        ]);
        return;
      ...
      try {
        ...
        const response = await submitUserDetails(
          chatbotId,
          `${updatedDetails.name},${updatedDetails.number},${updatedDetails.email}`
        );
        ...
        if (response.error) {
          ...
          setConversation((prev) => [
            ...prev,
            {
              text: response.error.message || "An error ocuureed.",
              isBot: true,
            },
          ]);
          return;
        ...
        setConversation((prev) => [
          ...
          {
            text: "Thank you for providing your details. Your query has been registered.",
            isBot: true,
          },
        ]);
  ...
  const handleReviewSubmit = async (detail, isSatisfaction = false) => {
    ...
    if (currentStep === 5 && isSatisfaction) {
      ...
      setConversation((prev) => [
        ...prev,
        {
          text: `Ratings: ${visualStars} (${detail}/5)`,
          isBot: false,
          isUser: true,
        },
      ...
      try {
        const response = await submitSatisfaction(
          chatbotId,
          updatedReviewDetails.satisfactionLevel
        );
      } catch (error) {
  ...
  const handleSubmitCallbackPreference = async (preference) => {
    try {
      ...
      setConversation((prev) => [
        ...prev,
        {
          text: "Callback preference submitted successfully.",
          isBot: true,
          options: [],
        },
      ...
      setConversation((prev) => [
        ...prev,
        {
          text: "Error submitting callback preference. Please try again later.",
          isBot: true,
          options: [],
        },
      ]);
  ...
  const handleTerminateResponse = async (chatbotId, option) => {
    try {
      ...
      if (option === "Y") {
        ...
        setConversation((prev) => [
          ...prev,
          { text: reconnectMessage, isBot: true, options: [] },
        ]);
      } else if (option === "N") {
        ...
        setConversation((prev) => [
          ...prev,
          { text: thankYouMessage, isBot: true, options: [] },
        ]);
        setTimeout(() => {
          setIsOpen(false);
        }, 2000);
      ...
      setConversation((prev) => [
        ...prev,
        {
          text: "Error sending your response. Please try again later.",
          isBot: true,
          options: [],
        },
      ]);
      ...
      <div className="chatbot-container">
        ...
        {isOpen && (
          <div className="chatbot-frame">
              ...
              {conversation.map(({ text, isBot, options }, index) => (
                <div key={index}>
                  <div className={`chatbot-message ${isBot ? "bot" : "user"}`}>
                    <i
                      className={`icon ${
                        isBot ? "fas fa-robot" : "fas fa-user"
                      }`}
                    ></i>{" "}
                    {/* Icon class based on isBot */}
                    {text}
                  </div>
            ...
            {currentStep === 4 &&
              !conversation.some(
                (item) => item.text === "Your Callback Preference Submitted"
              ) && (
                <div className="callback-preference">
                  <button
                    onClick={() =>
                      handleSubmitCallbackPreference("Yes Please!")
                    }
                  >
                    Yes{" "}
                  </button>
                  <button
                    onClick={() => handleSubmitCallbackPreference("No Please!")}
                  >
                    No
                  </button>
                </div>
                  ...
                  {[1, 2, 3, 4, 5].map((rating) => (
                    <button
                      ...
                      className={`star-button ${
                        currentSliderValue >= rating ? "filled" : ""
                      }`}
                      onClick={() => handleReviewSubmit(rating, true)}
                    >
                      ★
